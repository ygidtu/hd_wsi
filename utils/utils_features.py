...
def generate_nuclei_map(x, slide_size=None, n_classes=None, use_scores=False):
    ...
    if slide_size is None:
        h, w = int(math.ceil(x['boxes'][:, [1, 3]].max().item()) + 1), int(
            math.ceil(x['boxes'][:, [0, 2]].max().item()) + 1)
    else:
        h, w = slide_size
    ...
    if h0 < grid and w0 < grid:  # for small slide: h, w < grid
        ...
        for i in range(math.ceil(h0 / grid)):
            ...
            for j in range(math.ceil(w0 / grid)):
                ...
                row.append(
                    F.adaptive_avg_pool2d(pts[None], (round(grid * scale_factor), round(grid * scale_factor)))[0])
    ...
    for k, r in radius.items():
        if np.isnan(r) or r <= 1.:
            ...
            try:  # RuntimeError: std::bad_alloc with torch 1.9 and 1.10 on cpu
                ...
                logger.info(f"Use scipy for convolution.")
...
def polygons2mask(polygons, shape, scale=1.):
    ...
    polygons = [((np.array(_) * scale)).astype(int)
                for _ in polygons]
...
def random_patches(N, patch_size, polygons, image_size=None, scores_fn=None,
    ...
    if image_size is None:
        w, h = np.concatenate(polygons).max(0).astype(int)
    else:
        w, h = image_size
    ...
    patches = np.hstack([coords.astype(int), np.tile(patch_size, (pool_size, 1))])
    ...
    if scores_fn is None:  # use mask_region/patch_size as score
        ...
        scores = np.array([masks[y0:y0 + dh, x0:x0 + dw].sum() / dw / dh for x0, y0, dw, dh in patches.astype(int)])
    else:
        scores = np.array([scores_fn(_) for _ in patches.astype(int)])
    ...
    if polygons:
        ...
        for x0, y0, w, h in patches:
            ...
            if keep.sum() < 10 or core_nuclei.sum() < 10:
                logger.warning(
                    f"patch: [{x0}, {y0}, {x0 + h}, {y0 + w}] is ignored, less than 10 nuclei in this patch.")
                continue
            else:
                logger.info(f"Analyzed patch: [{x0}, {y0}, {x0 + h}, {y0 + w}], with {core_nuclei.sum()} tumor inside.")
        ...
        logger.info(f"Analyzed {k_patch} randomly selected patches (size={patch_size}).")
    else:
        logger.warning(f"can't find valid polygons (>=0.05*roi_area) in masks. ")
